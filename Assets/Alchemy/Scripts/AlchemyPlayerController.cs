...
public class AlchemyPlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(m_CameraMain.ViewportPointToRay(new Vector3(0.5F, 0.5F, 0)), out hit, 2f, m_LayerMask))
        {
            ...
            {
                AlchemyActionController _AC = hit.collider.GetComponent<AlchemyActionController>();

                if (_AC != null)
                    _AC.DoAction();
            }

            AlchemyUIController.m_Instance.ShowActionText(true);
        }
        else
            AlchemyUIController.m_Instance.ShowActionText(false);
    }
