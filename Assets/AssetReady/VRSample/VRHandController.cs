...
{
    VRIGrabable target = null; //OnTrigger에 들어온 대상
    VRIGrabable grabObj = null; //스페이스를 눌러서 잡은 대상
    ...
    private void OnTriggerEnter(Collider other)         //트리거 안에 들어왔을 때 호출 됨
    {
        ...
        if(other.GetComponent<VRIGrabable>() != null)     //충돌한 오브젝트에 IGrabable이 있으면,
        {
            Debug.Log("GrabTarget : " + other.gameObject.name);
            target = other.GetComponent<VRIGrabable>();   //IGrabable target에 넣어준다.
        }
    }
